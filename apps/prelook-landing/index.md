...
极速预览，无需等待。鼠标停在链接上的瞬间，目标页已经开始加载——先预览，再决定要不要打开。

## 功能特性

沉浸式浏览，不必来回切换。十二项能力，全部本地运行，不上传任何数据。

- **链接预览**（核心）：悬停链接即浮出内容预览，是否打开由你决定。（极速预览 / 见后再开）
- **链接预热**：悬停瞬间即开始预取目标页，预览与打开更快。（Speculation Rules / 原生预取）
- **阅读模式**：无法内嵌的网站自动切换为无干扰阅读视图。（禁嵌兜底 / 正文提取）
- **多窗口预览**：最多同时打开 6 个预览窗，左右对比阅读。（最多 6 窗 / 并排对比）
- **侧边栏模式**：预览窗通高贴边，侧栏堆叠不遮挡正文。（通高停靠 / 左右任选）
- **划词搜索**：选中文字，一键 Google / Bing / 百度 / DuckDuckGo。（4 大引擎 / 选中即搜）
- **AI 搜索**：DeepSeek / 豆包 / Kimi / Perplexity，选完即问。（4 家引擎 / 选中即问）
- **追踪参数清理**：自动剥掉 utm、fbclid 等追踪参数，还原中转壳里的真实地址。（剥离 utm / 还原真实链接）
- **链接风险提示**：仿冒域名、乱码域名、文字与目标不符时在标题栏给出提醒。（仿冒域名 / 文字不符）
- **四种触发方式**：悬停、Alt+悬停、长按、拖动，按习惯任选。（Alt 组合 / 长按拖动）
- **主题与外观**：深浅色、主题色、窗口配色、位置尺寸与背景模糊随心调。（深浅色 / 窗口配色）
- **隐私本地化**：无账号、无服务器，数据不出本机。（无账号 / 无服务器）
...
Prelook 完全免费、无账号、无服务器。如果它帮到了你，欢迎扫码赞助，支持后续开发。项目靠赞助维持（微信、支付宝、Ko-fi），赞助纯粹出于自愿。
