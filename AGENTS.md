...
| `pnpm landing:prelook`                              | 官网开发服务器（Vite，http://127.0.0.1:4173 ，**根目录是 `apps/prelook-landing`**）：css 改动就地热更新（换掉 `<link>`，不刷新页面、不丢滚动位置），js/html 改动整页刷新 |
...
apps/
  <name>/                    # 插件本体，包名 @extensions/<name>；工程根 = 此目录，@/ 别名指到这里
    ...
    public/_locales/         # 界面词条 zh_CN/messages.json + en/messages.json（浏览器原生 i18n，WXT 原样打进产物根目录的 _locales/）
  ...
  <name>-landing/            # 落地页，包名 @extensions/<name>-landing，运行期零依赖（vite 只是 devDependency）
...
固定状态只存在于 DOM 生命周期内（页面跳转/刷新即消失），不落存储。按钮外观由 `.tp-pin` / `.tp-on` 控制：固定时图标转正并染成主题色，未固定时旋转 45°，所以两个状态除了颜色还有形状差异。`syncHeaderButtons()` 统一维护三个按钮的 `title` 与 `aria-pressed`，`refreshTitlesAndBadges()` 就是遍历它（词条 `preview.pin` / `preview.unpin`，经 `browser.i18n` 取，语言跟随浏览器、会话内不变）。
...
- **单位约定**：延迟类设置**存储永远是毫秒**（`hoverDelayMs` / `longPressMs`，定时器直接用），只有设置面板的滑块与读数换成秒（`App.vue` 里两个 writable computed 做 `×1000 / ÷1000` 换算，`toFixed(2)` 去浮点噪声），这样不需要迁移老数据。预览窗宽高支持两种单位（`sizeUnit` = percent / px，默认 percent），**两套值各自独立记忆**：百分比存 `width`/`height`（20–100 视口百分比），像素存 `widthPx`/`heightPx`（`WINDOW_PX_MIN/MAX` = 200–2000），切单位不换算、不覆盖对方；`clampWindowPercent()` 会把历史遗留的像素值（>100 只可能是 px）按固定参考视口 `LEGACY_VP_W/H`（1440×900）折算一次——**百分比↔px 换算一律用这对参考视口，不能用 `innerWidth`**，因为设置面板的视口不是被浏览的页面（`clampSettings` 里对 `sizeUnit==="px"` 且缺少 px 字段的存量数据做一次性 seed 迁移）。
...
- 枚举取值：`triggerMode` = hover / altHover / longPress / drag（click / altClick 已移除，`clampSettings` 对存量坏值回落到 `DEFAULT_SETTINGS.triggerMode`）；`position` = top-left / bottom-right / bottom-left / top-right / center-top / center / center-bottom / sidebar（link / mouse 已移除，`clampSettings` 对存量旧值回落到 `DEFAULT_SETTINGS.position`）；`sidebarSide` = left / right；`speculationMode` = off / prefetch / prerender；`powerSaver` = auto / on / max / off。（`language` 设置项已随原生 i18n 迁移移除，`clampSettings` 会把它从存量数据里剥掉。）`powerSaver` 的夹取不能照抄"坏值回落到最省事的那档"：它每个取值都要拿去比较，回落到 `off` 会让省电功能静默失效，所以坏值一律回落到 `DEFAULT_SETTINGS.powerSaver`。
...
- **划词翻译**（`TRANSLATE_ENGINES` = google / bing）的落点由引擎表里的 **`openIn`** 决定，它不是偏好而是站点事实：**`translate.google.com` 回 `X-Frame-Options: SAMEORIGIN`，永远进不了 iframe，所以 google 是 `openIn: "tab"`**（走 `openTab()`，和 Web/AI 搜索一样受理「在后台标签页打开」），**bing 是 `openIn: "preview"`** 才在预览窗里打开（这也是该功能最初的形态）。选中文字由 `%s` 塞进 URL，目标语言由 `%t` 取（`browser.i18n.getUILanguage()` 以 zh 开头 → 中文，否则英文；源语言一律 auto），两家对同一目标语言的写法不同（`tl=zh-CN` / `to=zh-Hans`），所以码表挂在各自的 `lang` 上——**每条引擎都必须给 `lang`，模板里的 `%t` 就是从它取的**。`openIn: "preview"` 的窗口还带一个 `translate` 标记：`loadFlow` 判定 `!canEmbed`（以及 iframe 8s 超时走的 `fallbackToReader()`）时**必须跳过阅读模式**——翻译站是应用不是文章，阅读模式只会把应用自己的外壳当正文抓出来——改为 `showError(win, "preview.embedBlocked")`：一张说明卡片 + 复用既有的「在新标签页打开」按钮；这是**给 Bing 万一改主意的兜底**，Google 那条走不到这里。引擎选项是设置面板「划词」tab 里的单选（`translateEngine`，和 `searchEngine` 同款，下面配一条 `selection.translateHint` 说明两家落点不同），引擎名走词条 `translate.<id>`（带「翻译/Translate」后缀，所以不像 SEARCH/AI 表那样用字面 label）；默认引擎仍是 **bing**（默认值得能真的在预览窗里渲染出来），有道那条已删，存量数据里的 `translateEngine: "youdao"` 由 `clampSettings` 回落到默认。
- **「文字已复制」提示**：AI 引擎里 Kimi/豆包/DeepSeek 的模板没有 `%s`，`selection.ts` 的 `open()` 会先把选中文字写进剪贴板、再打开各自主页，**并在工具栏原位弹一条 `selection.aiCopied`**（「已复制选中文字，在新标签页粘贴即可」）——新标签页是个空白输入框，没有任何迹象表明文字已经就位，这条提示补的就是这个断层；模板带 `%s` 的引擎（Perplexity、Web 搜索）不复制也不提示。落地方式是 `PreviewSystem.notice(key, x, y, params?)`（原 `showNotice`，DOM 元素改名 `noticeEl`，和「预览窗全被固定」那条提示共用同一个 toast，`pointer-events: none`、2.6s 自动消失），由 `SelectionDeps.notify` 暴露给工具条；坐标必须在 `hide()` **之前**测量——工具条一隐藏就量到 0×0。
- **划词条的指向小箭头**：`.tp-sel::after` 是一个 10px 的旋转方块（`.tp-arrow-down` 挂下边框+右边框、`.tp-arrow-up` 挂上边框+左边框，另两边宽 0——否则会在条子内部画出一道线），**填充与描边都走 `inherit`**，所以深色主题不需要额外规则（`background`/`border-color` 跟着 `.tp-sel` 走）。横向位置由 `show()` 写在 `--tp-arrow-x` 上，值为**相对条子左边缘**的偏移；夹取下限是 `barHeight / 2 + 8`（半个条高就是胶囊圆角的半径，箭头最左点又外扩约 7px，贴着圆角会把一小片没有描边的白斜角戳到轮廓外面），所以条子特别窄时退化成居中（`Math.min(…, bw / 2)`）。竖向上条子本来就在选区上方/下方留 8px，箭头探出约 6px，剩 1–2px 缝。`y < 8` 翻到下方时同时翻转箭头方向，两个类是互斥 toggle，别只写一个。
- `skipImages`（默认 false，「预览窗」tab 开关）在 content 的 `anchorHref()` 里过滤图片链接（只看 href 路径是否以图片扩展名结尾，见 `isImageLink()`），悬停/长按/拖拽一律不触发；右键菜单「在预览窗打开」走 `anchorInfoFor()`，不受此开关影响。**判据里刻意不含「链接内含 `<img>`」**：卡片式信息流（如小黑盒 `xiaoheihe.cn/app/bbs/home`）把整条帖子连同缩略图包在一个 `<a>` 里，按「含图即跳过」会让整条信息流全部失效。
...
- **设置面板的 radio 组必须带 `name`**（triggerMode / position / sidebarSide / speculationMode）：`name` 才让浏览器把它们当原生单选组，方向键切换、读屏播报"N 选 1"都靠它。缺 `name` 时 Vue 仍会在 re-render 时回写 `checked`，鼠标点击看不出问题，但原生分组语义没了。触发方式那一组是 `.seg-btns` 分段按钮（`input` 视觉隐藏 + `label` 上色，选中态用 `settings.x === 值` 绑 `.on`；**六个选项，用的是 3 列网格、2 行**），其余仍是原生圆点。
- **新增界面文案**：词条在 `public/_locales/zh_CN/messages.json` 与 `en/messages.json`，经 `browser.i18n` 读取（`utils/i18n.ts` 只是薄封装：查表、`|| key` 兜底、把 `{值}` 参数代入 `$1` 位置占位）。调用点仍写扁平点号 key（`t('preview.close')`），**消息名只允许 `A-Za-z0-9_@`，所以 `_locales` 文件里的键是点号换下划线后的形式**（`preview_close`），封装在运行时做同一替换——改任何一边都要同步。zh_CN 与 en 必须同时补齐（CI「Locale key parity」查键名合法、两边对齐、message 非空）。语言**不能运行时切换**：`browser.i18n` 跟随浏览器 UI 语言，匹配不到回落 `default_locale`（zh_CN）；这正是旧 `language` 设置项被移除的原因，别再往设置里加语言开关。**locale 目录名与 `default_locale` 必须是下划线形式 `zh_CN`**：Chrome 会直接拒绝 `zh-CN`（报错还很迷惑——"使用了本地化但未指定 default_locale"），web-ext 对 `zh_CN` 也放行，两端唯一安全的写法就是它。消息文本里的字面 `$` 必须写成 `$$`。`wxt prepare` 会把消息名生成进 `getMessage` 的字面量联合类型，动态 key 只能 `as` 断言。manifest 的 `description` 用 `__MSG_extDescription__`（name 是品牌词、双语相同，保持字面量）。sidepanel 的 `<html lang>` 与 `<title>` 在 `main.ts` 里按 `browser.i18n.getUILanguage()` 重打。测试台（`test/repro-hover.html`、`test/demo/mock.js`）的 chrome mock 必须提供 `i18n.getUILanguage` 与同步的 `getMessage`（两者都从 `.output/chrome-mv3/_locales/zh_CN/messages.json` 同步 XHR 读真实词条）。
...
hero 那段视频**要求服务端支持 Range 请求**：没有 Range，浏览器拖不动进度条，Safari 更会直接拒绝播放一个服务端不声明 `accept-ranges` 的媒体文件。这一条很容易漏——`python -m http.server` 就没有（用它验证时会看到"能播但拖不动刻度"）。现在本地预览走 `pnpm landing:prelook`（Vite，配置在 `apps/prelook-landing/vite.config.js`）：Vite 自己处理分段请求（`206` + `Content-Range`），但**只对 206 写 `Accept-Ranges`**，所以那份配置里有一个按扩展名给 200 全量响应也补上 `Accept-Ranges: bytes` 的小中间件——**别顺手删掉它**，删了 Safari 就不认这段视频（Chrome 无所谓，它不看这个头也会发 Range 请求，所以本地用 Chrome 验证时发现不了）。`video/mp4` 的 MIME 由 Vite 自带的 mime 表给出，不用另外配。

landing **只把 Vite 当开发服务器**，没有构建步骤：`index.html` / `privacy.html` + `css/` `js/` `assets/` 原样就是部署物（整目录直传），`vite.config.js` 里 `root` 就是这一层，所以 dev 与线上看到的是同一批文件、同一套相对路径（`./assets/...`）。两个刻意的后果：**不要给它加 `build` 脚本**——根 `pnpm build` 是 `pnpm -r --if-present build`，一加就会连带产出 `dist/`，等于凭空多出一条和"整目录直传"并行的部署路径，迟早有人部署错那一份；`appType: 'mpa'`（多页静态站，未知路径 404 而不是回落到 index.html）。热更新行为：`css` 走 css-update（Vite client 就地换掉 `<link>`，**不刷新页面**，所以调样式时不丢滚动位置），`js` / `html` 改动走整页刷新。
...
`background.ts` 里建三层菜单：`prelook-root`（`contexts: ['link']`，标题 `menu.root`）下挂 `prelook-open-popup` 与 `prelook-open-sidebar`。Chrome 不会本地化菜单标题，所以标题由 `translate()`（`browser.i18n`，跟随浏览器语言）生成，并在 `onInstalled` 时重建（`removeAll()` 后再 `create()`，避免 id 重复报错；原生 i18n 会话内不会变，不再有"语言变化触发重建"那回事）。
