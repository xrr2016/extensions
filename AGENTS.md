...
- **Prelook** — 悬停链接预览扩展（MV3，Chrome/Edge/Firefox），WXT 0.21 + Vue 3。**产品曾用名 TabPeek，2026-09-16 整体改名**——改名撞车是因为 Chrome Web Store 上已有一款同名扩展（`hcheung-code`，做的是悬停预览标签组，站 site tabpeek.com），所以任何带 `Tab` 前缀的名字都会被当成那款产品。核心能力：悬停/Alt+悬停/点击/长按触发页面内悬浮预览窗（iframe 优先、禁嵌站点自动切阅读模式）、链接预热、划词搜索（普通 + AI + 翻译，翻译在预览窗里打开）、全量设置面板（侧边栏）、多窗口预览（最多 6 窗）。**全部功能免费**，没有 Pro/授权码/付费体系，只在侧边栏设置面板与 landing 放了爱发电 + Patreon 赞助入口。
...
- **划词翻译**（`TRANSLATE_ENGINES` = google / bing）是唯一走**预览窗**的划词动作：`selection.ts` 的「翻译」按钮调 `deps.openPreview(url, rect, { translate: true })`，选中文字由 `%s` 塞进 URL，目标语言由 `%t` 取（`browser.i18n.getUILanguage()` 以 zh 开头 → 中文，否则英文；源语言一律 auto），两家对同一目标语言的写法不同（`tl=zh-CN` / `to=zh-Hans`），所以码表挂在各自的 `lang` 上——**每条引擎都必须给 `lang`，模板里的 `%t` 就是从它取的**。两个坑：①**`translate.google.com` 回 `X-Frame-Options: SAMEORIGIN`，只有 Bing 能嵌**（Google 那条 URL 的预填参数本身没问题，被浏览器拦掉的只是 iframe），所以带 `translate` 标记的窗口在 `loadFlow` 判定 `!canEmbed`（以及 iframe 8s 超时走的 `fallbackToReader()`）时**必须跳过阅读模式**——翻译站是应用不是文章，阅读模式只会把应用自己的外壳当正文抓出来——改为 `showError(win, "preview.embedBlocked")`：一张说明卡片 + 复用既有的「在新标签页打开」按钮；②默认引擎因此是 **bing 而不是 google**：默认值得能真的在预览窗里渲染出来。引擎选项是设置面板「划词」tab 里的单选（`translateEngine`，和 `searchEngine` 同款），引擎名走词条 `translate.<id>`（带「翻译/Translate」后缀，所以不像 SEARCH/AI 表那样用字面 label）；有道那条已删，存量数据里的 `translateEngine: "youdao"` 由 `clampSettings` 回落到默认。
