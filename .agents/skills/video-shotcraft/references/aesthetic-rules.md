...
### R4. 卡点约束时机不放大幅度：全画面级节拍冲击全片 ≤3 处，默认只动元素层
规则：强节奏 BGM 片"切点落拍"不等于"逐拍打画面"——作用于整画面/相机层的冲击（整画面 scale 泵/震屏/闪帧/负片帧）视同大 slam 管理：全片 ≤3 处、钉 music-beat-sync §2b 的最强 hit 清单、相邻两处间隔 ≥16 拍；其余拍点动效只作用主角元素层；节拍泵类连打单次 ≤4 拍、全片 ≤1 次。kick 命中表是候选池不是触发器。**计数单位是一次手法/set-piece 使用，不按内部子事件数**：一张卡的一次应用（paparazzi 三连闪、strobe 黑帧串、trailer-bumper 三刀切）算一个名额，卡自身标注的连发结构与全片次数上限照旧生效，不因本条被逐帧重复计数。
判例：用户反馈（2026-08-22）："如果给音乐，镜头会随节奏抖动，影响观感"——根因是把 music-beat-sync 的 kick→冲击映射执行成了逐拍全画面泵：强鼓点曲 kick 几乎每拍都有，逐拍"被砸"叠上相机层就是节拍频率的画面抖动。与 Q3 同族：Q3 封噪声性抖动（handheld），本条封节拍性抖动。
自检：高能段任取连续 8 拍逐帧回放：整画面 scale/位移是否在逐拍脉冲？数全片作用于整画面/相机层的冲击：超过 3 处了吗？每处对得上最强 hit 清单吗？
...
### S5. 音画对齐补两项偏移：输出音轨偏移逐渲染管线实测统一扣，音源峰值滞后逐文件量
规则：渲出成片的整条音轨相对视频有固定滞后，根因在输出编码/封装链路（AAC encoder priming，48kHz 典型 2048 samples ≈1.28f@30fps；Remotion 跟踪 remotion-dev/remotion#7099），不是 `<Audio>`/`Sequence.from` 的行为——按渲染管线（Remotion 版本 + codec + sample rate + 容器）实测记档（渲出成片抽音轨，拿 2–3 个尖锐 SFX 原文件做归一化交叉相关，残差一致才算量准），补偿在渲染层的 from 计算式统一扣除，不逐条改帧号；音源自身的峰值滞后（峰值不在文件头）逐文件量。统一公式 `from = 目标峰值帧 − 峰值滞后 − 输出偏移`，方法与边界（截断型音效窗口不加长、片头 2f 内无法全补）见 sound-design §4.6。
判例：AiflPromo 实测 click 滞后 2032 samples、impact 2066 samples（≈1.27–1.29f），与 AAC priming 指纹吻合，渲染品 elst 未声明裁剪垫头；promo-ink 时代实测过 4f（≈133ms，远超 priming 量级，根因未验证）——常数不可跨管线搬运。不补时两项叠加最重一击晚 0.27s，明显读作音画不同步；卡点片里差 2–3 帧就足以让合理的卡点动效被读作无端抖动（2026-08-22 用户反馈的成因之一）。
自检：本管线的输出音轨偏移实测过吗（Remotion 版本/codec/采样率/容器随值记录在哪）？补偿是 from 计算式统一扣的还是散落在各条帧号里？带明显攻击峰的音源，峰值滞后量过了吗？
